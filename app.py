...
with mysql.connector.connect(host=host,password=password,db=db,user=user,port=port) as conn:
    ...
    cursor.execute('CREATE TABLE if not exists admincreate (email varchar(40) NOT NULL,username varchar(100) NOT NULL,password varbinary(10) NOT NULL,address text NOT NULL,accept enum('on','off') DEFAULT NULL,dp_image varchar(50) DEFAULT NULL,ph_no bigint DEFAULT NULL,PRIMARY KEY (email),UNIQUE KEY ph_no (ph_no)')
